...
p<- 150        #the number of covariates p=100, 200
...
H[7,14]<- 1; H[8,15]<- 1; H[9,16]<- 1
# restriction bounds
U<- c(rep(-1.7, times=3), rep(3.3, times=3), rep(1.5, times=3))  #upper bound
L<- c(rep(-2.4, times=3), rep(2.5, times=3), rep(0.5, times=3))  #lower bound

#covariance matrix for each group
first_col <- c(1, rep(rho, 5))
Sb2<- toeplitz(first_col)

# covariance matrix for 4 groups
first_col2 <- c(1, rep(0.3, 17))
Sb1<- toeplitz(first_col2)
Sb1[1:6, 1:6] <- Sb2
Sb1[7:12, 7:12] <- Sb2
Sb1[13:18, 13:18] <- Sb2
...
for (m in 1:nrep) {
  ...
  #-----------------------restricted model------------------------#
  #initilizing values
  beta1<- c(rep(-1.8, times=6), rep(2.7, times=6), rep(0.7, times=6),  rep(0.3, times=132))
