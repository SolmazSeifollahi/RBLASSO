rm(list=ls())
setwd("C:/Users/Almustaqbal/Desktop/Old setting/old setting 2")
library(MASS)
library(statmod)
library(tmvmixnorm)
library(glmnet)
...
filename <- paste0("OLD low_setting_2_rho(rho=", rho,")", ".txt")
sink(filename)
round(output2, 4)
...
filename1 <- paste0("OLD (MSEy & RMSEy & MAEy)low_setting_2(rho=", rho,")", ".csv")
write.csv(cbind(MSEy, RMSEy, MAEy), filename1, row.names = FALSE)

filename2 <- paste0("OLD (MSEb & RMSEb  & MAEb)low_setting_2(rho=", rho,")", ".csv")
write.csv(cbind(MSEb, RMSEb, MAEb), filename2, row.names = FALSE)

filename3 <- paste0("OLD cerit low_setting_2(rho=", rho,")", ".csv")
write.csv(cbind(recerit,uncerit), filename3, row.names = FALSE)

filename4<- paste0("OLD RE low_setting_2(rho=", rho,")", ".csv")
write.csv(rebeta, file = filename4, row.names = FALSE)

filename5<- paste0("OLD Un low_setting_2(rho=", rho,")", ".csv")
write.csv(unbeta, file = filename5, row.names = FALSE)

filename6<- paste0("OLD NONz low_setting_2(rho=", rho,")", ".csv")
