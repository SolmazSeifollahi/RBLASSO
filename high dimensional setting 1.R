rm(list=ls())
setwd("C:/Users/Almustaqbal/Desktop/Old setting/high setting/settign 1")
library(MASS)
library(statmod)
library(tmvmixnorm)
library(glmnet)
...
p<- 50        #the number of covariates p=100, 200
...
U<- c(2.5, 2.5, 2.5, 4.5)         #upper bound
L<- c(1.5, 1.5, 1.5, 3.5)   #lower bound
...
filename <- paste0("OLD high_setting_1(rho=", rho,")", ".txt")
sink(filename)
round(output2, 4)
...
filename1 <- paste0("OLD (MSEy & RMSEy & MAEy)high_setting_1(rho=", rho,")", ".csv")
write.csv(cbind(MSEy, RMSEy, MAEy), filename1, row.names = FALSE)

filename2 <- paste0("OLD (MSEb & RMSEb  & MAEb)high_setting_1(rho=", rho,")", ".csv")
write.csv(cbind(MSEb, RMSEb, MAEb), filename2, row.names = FALSE)

filename3 <- paste0("OLD cerit high_setting_1(rho=", rho,")", ".csv")
write.csv(cbind(recerit,uncerit), filename3, row.names = FALSE)

filename4<- paste0("OLD RE high_setting_1(rho=", rho,")", ".csv")
write.csv(rebeta, file = filename4, row.names = FALSE)

filename5<- paste0("OLD Un high_setting_1(rho=", rho,")", ".csv")
write.csv(unbeta, file = filename5, row.names = FALSE)

filename6<- paste0("OLD NONz high_setting_1(rho=", rho,")", ".csv")
